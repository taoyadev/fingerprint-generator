...
/**
 * Zero-dependency CLI for the Fingerprint Generator.
 *
 * The previous implementation relied on runtime dependencies that were not
 * installed in the project. This version keeps the surface extremely small,
 * relying solely on core Node.js APIs while still offering the same commands.
 */

import { writeFileSync, existsSync, mkdirSync } from 'fs';
import { dirname, resolve } from 'path';
import { FingerprintGenerator } from '../FingerprintGenerator';
import { BrowserName, DeviceType, FingerprintOverrides, GenerationOptions } from '../types';

type FlagMap = Record<string, string | boolean>;
type GeneratedResult = Awaited<ReturnType<FingerprintGenerator['generate']>>;
...
function buildGenerator(flags: FlagMap): FingerprintGenerator {
  const seedValue = flags.seed ? Number(flags.seed) : undefined;
  const generatorOptions: { randomSeed?: number } = {};
  if (typeof seedValue === 'number' && Number.isFinite(seedValue)) {
    generatorOptions.randomSeed = seedValue;
  }
  return new FingerprintGenerator(generatorOptions);
}

function buildGenerationOptions(flags: FlagMap): GenerationOptions {
  ...
  if (browserFlag) {
    ...
    const version: string = getStringFlag(flags, 'version') ?? '120';
    const majorToken = version.split('.')[0] || version;
    overrides.browser = {
      name: browserFlag as BrowserName,
      version,
      majorVersion: parseInt(majorToken, 10) || 120
    };
    options.browsers = [browserFlag as BrowserName];
    hasOverride = true;
  }
  ...
  if (deviceFlag) {
    ...
    overrides.device = {
      ...(overrides.device ?? {}),
      type: deviceFlag as DeviceType
    };
    options.devices = [deviceFlag as DeviceType];
    hasOverride = true;
  }
...
async function handleGenerate(ctx: CommandContext) {
  ...
  let result;
  if (browserFlag) {
    result = await generator.generateForBrowser(browserFlag as BrowserName, versionFlag, options);
  } else if (deviceFlag) {
    const platform = platformFlag ?? (deviceFlag === 'mobile' ? 'android' : 'windows');
    result = await generator.generateForDevice(deviceFlag as DeviceType, platform, options);
  } else {
...
async function handleBatch(ctx: CommandContext) {
...
async function handleStats(ctx: CommandContext) {
  const generator = buildGenerator(ctx.flags);
...
async function handleUpdate(ctx: CommandContext) {
  const generator = buildGenerator(ctx.flags);
  await generator.updateData();
}

function printFingerprint(result: any, verbose: boolean) {
...
function printValidation(report: any) {
  console.log('\nValidation');
...
function printBatchSummary(summary: any) {
...
function buildSummary(results: GeneratedResult[], total: number, totalTimeMs: number) {
...
function saveOutput(target: string, data: any) {
...
function assertValid(label: string, value: string, allowed: Set<string>) {
