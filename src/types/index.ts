/**
 * Core type definitions for the statistical fingerprint generator
 */
...
export interface FingerprintConstraints {
  browsers?: BrowserName[] | { name: BrowserName; minVersion?: number; maxVersion?: number }[];
  devices?: DeviceType[];
...
// Conditional probability mapping (can be nested or flat)
export interface ConditionalProbability {
  [condition: string]: ProbabilityDistribution | any;
}

// Probability distribution types (using interface for backward compatibility)
...
export interface GenerationOptions extends FingerprintConstraints {
  ...
  forceRegenerate?: boolean; // Skip cache and force new generation
}
...
export class BayesianNetworkError extends Error {
  constructor(
    message: string,
